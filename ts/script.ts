...
let values: { id: number; name: string; number: number; difference: number }[] = [];


function startGame() {
    if (container) {
        let startInput = document.querySelector('.start input') as HTMLInputElement;
        let nplayer = +startInput?.value;
        container.innerHTML = '';
...
function getValue(i) {
    let playername = document.querySelector('#player' + i) as HTMLInputElement;
    let playernum = document.querySelector('#number' + i) as HTMLInputElement;
    let player = {
        id: i,
